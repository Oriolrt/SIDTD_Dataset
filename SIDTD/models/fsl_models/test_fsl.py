...
from utils.data_tools import TaskSamplerPath, TaskSamplerCoAARCPath, EasySetRandomPath, EasySetCoAARCRandomPath
...
from smoothAP_loss import SmoothAP
...
def setup_trans_fg(args):
    ...
    img_size = 299
    #prerapre model
    #model = VisionTransformer(config, img_size, zero_head=True, num_classes=132, smoothing_value=0.0)
    ...
    if args.pretrained_dataset == 'imagenet':
        ...
        model.load_from(np.load('./transfg_pretrained/imagenet21k+imagenet2012_ViT-L_16.npz'))
    elif args.pretrained_dataset=='banknotes':
        print('Banknotes pretrained')
        model.load_state_dict(torch.load('./banknotes_pretrained/banknotes_trans_fg_best_accuracy_n1.pth'))
    elif args.pretrained_dataset=='dogs':
        print('Dogs pretrained')
        model.load_state_dict(torch.load('./dogs_pretrained/new_dogs_simp_trans_fg_best_accuracy_n0.pth'))
    elif args.pretrained_dataset == 'df20m':
        print('DF20M pretrained')
        model.load_state_dict(torch.load('./df20m_pretrained/new_DF20M_simp_trans_fg_best_accuracy_n0.pth'))
    #model.load_from(np.load('/home/soteria/benchmark_soteria/transfg/transfg_pretrained/imagenet21k+imagenet2012_ViT-L_16.npz'))
...
def setup_model_baseline(args, pretrained=True):
    ...
    if args.model == 'resnet50':
        ...
        if args.pretrained_dataset != 'imagenet':
            if args.pretrained_dataset == 'banknotes':
                path_model = './banknotes_pretrained/banknotes_ResNet_50_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'dogs':
                path_model = './dogs_pretrained/new_dogs_simp_ResNet_50_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'df20m':
                path_model = './df20m_pretrained/new_DF20M_simp_ResNet50_best_accuracy_n0.pth'
        ...
        setattr(model, last_layer, nn.Linear(num_ftrs,  2))

        if args.pretrained_dataset != 'imagenet':
            if args.pretrained_dataset == 'banknotes':
                path_model = './banknotes_pretrained/banknotes_vit_large_patch16_224_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'dogs':
                path_model = './dogs_pretrained/new_dogs_simp_vit_large_patch16_224_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'df20m':
                path_model = './df20m_pretrained/new_DF20M_simp_vit_large_patch16_best_accuracy_n0.pth'
            pretrained_dict = torch.load(path_model)
            model.load_state_dict(pretrained_dict)
        ...
        if args.pretrained_dataset != 'imagenet':
            if args.pretrained_dataset == 'banknotes':
                path_model = './banknotes_pretrained/banknotes_EfficientNet_b3_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'dogs':
                path_model = './dogs_pretrained/new_dogs_simp_EfficientNet_b3_kfold_rectified_best_accuracy_n0.pth'
            elif args.pretrained_dataset == 'df20m':
                path_model = './df20m_pretrained/new_DF20M_simp_EfficientNet_best_accuracy_n0.pth'
            pretrained_dict = torch.load(path_model)
            model.load_state_dict(pretrained_dict)
...
def main(args):
    ...
    for nb_rep in range(args.repetition):
        ...
        if args.model == 'coaarc':
            test_set = EasySetCoAARCRandomPath(args.dataset, test_metaclass, image_size=image_size, training=False)
        else:
            test_set = EasySetRandomPath(args.dataset, test_metaclass, image_size=image_size, training=False)
        ...
        model.load_state_dict(torch.load(f"/data/users/soteria/trained_models/fsl_model/{args.dataset}/{save_folder_name}/{args.model}/{args.name}_{args.model}_kshot_{args.k_shot}_pretrained_{args.pretrained_dataset}_iteration_{nb_rep}.pth"))
        ...
        if not os.path.exists(f"stats/github/test_split_{args.dataset}"):
            os.makedirs(f"stats/github/test_split_{args.dataset}")
        my_array = np.asarray([l_paths_query, reals, l_preds, p_preds]).T
        df = pd.DataFrame(my_array, columns = ['Path_image', 'Label_image','Pred_label_image','proba_roc_auc'])
        df.to_csv(f"stats/github/test_split_{args.dataset}/{args.model}_{args.name}_iteration_{nb_rep}_stats_per_image.csv", index=False)
        ...
        if os.path.isfile(f'stats/github/test_split_{args.dataset}/{args.model}_{args.name}_test_results.csv'):
            f_test = open(f'stats/github/test_split_{args.dataset}/{args.model}_{args.name}_test_results.csv', 'a')
            writer_test = csv.writer(f_test)
        else:
            f_test = open(f'stats/github/test_split_{args.dataset}/{args.model}_{args.name}_test_results.csv', 'w')
            # create the csv writer
            writer_test = csv.writer(f_test)
            header_test = ['iteration', 'n_shot', 'accuracy', 'roc_auc_score']
            writer_test.writerow(header_test)

        test_res = [nb_rep, args.k_shot, acc_tot, roc_auc_score_tot]
